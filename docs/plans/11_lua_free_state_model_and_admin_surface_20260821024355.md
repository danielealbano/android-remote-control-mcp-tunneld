...
- **Out-of-scope, untouched:** `go.sum` carries pre-existing uncommitted transitive-hash additions made before this plan; `go mod tidy` reverted them (not real dependencies) so the committed `go.sum` is unchanged.
- **US5 endpoint rename (post-review, user-requested):** the list endpoint was renamed `GET /api/v1/admin/tunnels` → `GET /api/v1/admin/tunnels/list`, so it reads as a proper sibling of `POST /api/v1/admin/tunnels/stats` (the bare `/api/v1/admin/tunnels` path no longer routes). Docs/README/comments updated to match.
- **Out-of-scope broken test fixed (CI):** `internal/attest/signers_test.go` `TestSignerAllowlistReloadFailureLogged` was a CI-only flake (a non-atomic truncate-write exposed a 0-byte window that reloads as a valid EMPTY set, dropping the last-known-good). Made the test replace the file atomically (temp + rename). Fixed per the "fix ANY broken test" rule; a latent production consideration (a transient empty read clobbering the security-critical set — fail-closed, self-healing) is surfaced separately, not changed here.

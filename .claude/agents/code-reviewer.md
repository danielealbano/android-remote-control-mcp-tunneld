...
- **Language idioms**: Code MUST follow the language's established best practices (per the language rule file(s) in `.claude/rules/`) and existing codebase conventions. You MUST flag violations.
...
- Access control / authorization MUST be enforced server-side, derived from the authenticated identity (e.g. a verified client certificate or an authenticated session/token, per the project's documented authentication model); the client MUST NEVER be trusted to scope its own access. You MUST flag missing or client-side-only authorization.

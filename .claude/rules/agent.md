...
(libraries, runtimes, platform services, open standards, default
...
- Runtimes and platforms the project builds on (language runtimes,
  operating systems, container engines, managed services, edge/CDN
  providers) — their default configurations, limits, timeouts,
  quotas, and behavior.
- Infrastructure and third-party services in the deployment path
  (reverse proxies, databases, message brokers, observability
  stacks) — their configuration semantics, defaults, and formats.
- Third-party libraries and dependencies — their public API names,
  signatures, behavior, default values at the pinned version.
- Open standards — RFC/IETF wire-format details, POSIX behavior,
  spec field names + semantics, well-known protocol constants.
...
- "Project X does Y" / "Library X supports Y" / "The default for X is Y"
  / "Spec X says Y" / "Tool X blocks Y" / "Version X added Y".
...
1. **You MUST identify an authoritative source for the claim BEFORE
   ...
   2. Official documentation site at the pinned version
      (pkg.go.dev for Go modules at the exact `@v...` tag; the
      vendor's official docs for platform/service limits; the
      canonical spec PDF/HTML for standards).
   3. The authoritative upstream headers/definition files for
      ABI/wire-format facts.
...
5. **"Tool X does Y"** style assertions (e.g., "proxy X strips
   header Y", "provider X times out after N seconds") MUST be
   backed by the actual upstream configuration / documentation /
   source — not by general memory of what the project "does." If
   you cannot point to a file/line/section in the upstream source,
   you have NOT verified the claim and MUST treat it as unverified
   per rule 3.
...
- You MUST ALWAYS stage and commit ALL `.claude/` changes on the current working branch, regardless of who made them.

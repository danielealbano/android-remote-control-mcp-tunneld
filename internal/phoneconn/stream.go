...
// httpDataStream splices one dial-back data stream: Read pulls phone→client bytes from the /api/v1/data
// request body; Write pushes client→phone bytes to the /api/v1/data response body (flushed). Close signals the
...
// readControlFrame reads one length-framed v1 control frame from r: [type:1][len:4 BE][payload].
